...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------
// 本文件全是在讲一个加载 GISAXS 信息的函数，参数：光线类型
Function LoadNewGISAXS(Beamline)
	...
	//-------------------Chargement de l'image gisaxs2D et d�finition de l'espace r�ciproque (qy,qz)-----------------------------------	
	// 根据光线种类不同，调用对应的load函数(定义在本文件下方)
...
//	alphaf=atan((p-DirectY)*resolutionY/distance-tan(alphai_detec*pi/180))*180/pi
	...
	// 目测cmtstr函数接收两个字符串参数，比较两者是否相同，否，返回0
	// 根据不同的光线种类绘出不同的按钮
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------
// 控制线性缩放，控制button6 button 66的可点击状态，存疑
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------
// 控制什么东西看不懂(一个二维数组，以及两个button的可点击状态)
